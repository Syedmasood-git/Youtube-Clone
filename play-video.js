...
const creatorsdetails = document.querySelector(".creatorDetails");
...
function videoName(videoId) {
  ...
  name.innerHTML = `
    ...
    <div class="viewsContainer">
      <div class="views">
        <p>${getViews(videoId[0].statistics.viewCount)} Views . ${getDate(
    videoId[0].snippet.publishedAt
  )}</p>
      </div>
      <div class="likeShare">
        <img src="./Assets/like.svg" /><span>${getViews(
          videoId[0].statistics.likeCount
        )}</span>
...
function creatorDetails(videoId) {
  ...
  creator.innerHTML = `<div class="one">
                          ...
                          <div id="subscribers" class="subscribers">
                            <div class="creatorName">
                              ...
                              <p>${getViews(
                                videoId.statistics.subscriberCount
                              )} Subscribers</p>
                            </div>
  ...
  creatorsdetails.appendChild(creator);
}
  ...
  videosList.forEach((videoID) => {
    ...
    suggested.innerHTML = `
          ...
          <div class="vidTitle">
            ...
            <p>${getViews(videoID.statistics.viewCount)} views . ${getDate(
      videoID.snippet.publishedAt
    )}</p>
          </div>
          </div>`;
...
async function fetchResults() {
  ...
  right.innerHTML = "";
  // const endpoint=`${baseUrl}/search?key=${apiKey}&q=${searchString}&part=snippet&maxResults=5`
  const endpoint = `${baseUrl}/videos?key=${apiKey}&regionCode=IN&chart=mostPopular&part=snippet,statistics&maxResults=20`;
  try {
    const response = await fetch(endpoint);
    const result = await response.json();
    renderVideosOntoUI(result.items);
